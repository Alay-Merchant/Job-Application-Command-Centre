...
const schema = z.object({ cvId: z.string().uuid(), rawText: z.string().min(1).max(40000) });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { data: existing } = await auth.supabase.from("cv_profiles").select("id").eq("id", cvId).eq("user_id", auth.user.id).single();
