...
const schema = z.object({ applicationId: z.string().regex(/^[a-z0-9]{15}$/i), questions: z.array(z.string().min(3).max(500)).min(1).max(8) });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { data: app } = await auth.pb.from("applications").select("*,job:jobs(*),cv:cv_profiles(*)").eq("id", applicationId).eq("user_id", auth.user.id).single();
    ...
    const { data: existing } = await auth.pb.from("application_kits").select("id,auto_answers").eq("application_id", applicationId).eq("user_id", auth.user.id).maybeSingle();
    ...
    const query = existing
      ? auth.pb.from("application_kits").update({ auto_answers: allAnswers, model: "gpt-4o-mini" }).eq("id", existing.id).eq("user_id", auth.user.id)
      : auth.pb.from("application_kits").insert({ user_id: auth.user.id, application_id: applicationId, cv_profile_id: app.cv.id, input_hash: "pending-kit", model: "gpt-4o-mini", auto_answers: answers });
